	...
	$('.add__to_cart').click(function(){
		...
		$.ajax({
			...
			beforeSend:function(){
				...
				_this.find('.fa__cart').html('<i class="fa fa-spinner fa-spin" aria-hidden="true"></i>');
			...
			complete:function(){
				...
				_this.find('.fa__cart').html('<i class="fa fa-cart-arrow-down" aria-hidden="true"></i>');
...
_CART.addCart=()=>{
	...
	$('body').on('click','.add__to_cart',function(e){
	...
	if(exists($('#form-checkout'))){
		...
		$('#form-checkout').validate({
	        ...
	        messages: {
				...
				'id_ward':{ required: lang.chua_chon_phuong_xa},
