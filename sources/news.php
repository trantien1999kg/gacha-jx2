<?php  if(!defined('_source')) die("Error");
    ...
    if(!empty($idl)){
        ...
        $row_list=$db->rawQueryOne("select id,ten_$lang,tenkhongdau_$lang as tenkhongdau,mota_$lang,noidung_$lang,options,mucluc,photo,type,cano_$lang from #_baiviet_list where hienthi=1 and type=? and id=? order by stt asc, id desc",array($type,$idl));
    ...
    if(empty($id) && empty($idl) && empty($idc) && empty($idi) && empty($ids)){
        ...
        $limit_type=($type!='san-pham') ? 'limit 0,10':'';

        $tintuc=$db->rawQuery("select *,tenkhongdau_$lang as tenkhongdau from table_baiviet where hienthi=1 and type=?{$subWhere} order by stt asc, id desc $limit_type",array($type));
